...
class _JackOrbState extends State<JackOrb> with TickerProviderStateMixin {
  ...
  void _startAutonomousSaccades() {
    ...
    _saccadeTimer = Timer.periodic(const Duration(milliseconds: 3200), (timer) {
      ...
      if (shouldGlance) {
        _targetOffsetX = (_rng.nextDouble() * 2 - 1) * maxOffset * 0.7;
        _targetOffsetY = (_rng.nextDouble() * 1.5 - 0.3) * maxOffset * 0.6;
      } else {
        _targetOffsetX = 0.0;
        _targetOffsetY = 0.0;
      }
  ...
  void _initSensors() {
    try {
      _accelSub = accelerometerEventStream().listen((AccelerometerEvent event) {
        if (!mounted) return;
        // Sensitivity scaled with size - no setState here to prevent 100Hz UI jank
    ...
    return GestureDetector(
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          // Smoothly interpolate eye tracking towards target on each display refresh frame
          if (!_springCtrl.isAnimating && _pitch == 0.0 && _yaw == 0.0) {
            _eyeOffsetX += (_targetOffsetX - _eyeOffsetX) * 0.12;
            _eyeOffsetY += (_targetOffsetY - _eyeOffsetY) * 0.12;
          }
